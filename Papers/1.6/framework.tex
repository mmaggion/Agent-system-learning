...
\begin{equation}
 ...
 \label{rho}
\end{equation}
for any Borel set  $A\subseteq\R_+$, to take into account the polynomial weights $s^2$ as appearing in \eqref{midpoint1}.
...
For functions $a$ in $X_{M,K}$ and for finite element spaces $V_N$ of continuous piecewise linear functions constructed on regular meshes of size $N^{-1}$ a simple sequence $(a_N)_{N \in \mathbb N}$  realizing \eqref{rate1} with $C_1=M$ is the piecewise linear approximation to $a$ which interpolates $a$ on the mesh nodes. For the approximation estimate \eqref{rate2} there are plenty of results
