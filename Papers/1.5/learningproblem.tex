...
Additionally we restrict for the moment our attention to the case where $\mu(t_0)$ is a discrete measure $\mu^N=\frac1N\sum_{i=1}^N\delta_{x_i}$ (we drop $t_0$ since it is now fixed), so that the inequality reads
\begin{align}
...
We hope that these examples convince the reader that the coercivity condition is actually verifiable in most of the cases. Indeed we showed that it holds for generic cases as well as for highly structured deterministic ones. In parctice we can numerically verify it to hold for any "intermediate" situation as well, see Section \ref{numcoer}, and from now on we assume it without further concerns. 
